...
namespace MultiThreading.Task3.MatrixMultiplier
{
    class Program
    {
        ...
        private static void CreateAndProcessMatrices(byte sizeOfMatrix)
        {
            ...
            var firstMatrix = new Matrix(sizeOfMatrix, sizeOfMatrix, true); //random init enable
            var secondMatrix = new Matrix(sizeOfMatrix, sizeOfMatrix, true); //random init enable
            ...
            IMatrix parallelMatrix = new MatricesMultiplierParallel().Multiply(firstMatrix, secondMatrix);
            Console.WriteLine("firstMatrix:");
            firstMatrix.Print();
            Console.WriteLine("secondMatrix:");
            secondMatrix.Print();
